...
window.addEventListener("scroll", () => {
  ...
  if (heroVisual) {
    heroVisual.style.transform = `translateY(${offset * -0.03}px)`;
  }

  if (meshStage) {
    meshStage.style.transform = `translateY(${offset * -0.06}px) rotate(${offset * 0.02}deg)`;
  }

  if (networkPanel) {
    networkPanel.style.transform = `translateY(${offset * -0.018}px)`;
  }
});
